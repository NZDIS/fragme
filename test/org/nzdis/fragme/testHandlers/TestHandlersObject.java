...
public class TestHandlersObject extends FMeObject implements ChangePermissionHandler, DelegateOwnershipPermissionHandler, DeletePermissionHandler {
	...
	public void deserialize(FMeObject serObject) {
		this.value = ((TestHandlersObject)serObject).getValue();
	}

	private static class Factory extends FragMeFactory {
		protected FactoryObject create() {
			return new TestHandlersObject();
		}
	}

	static {
		FragMeFactory.addFactory(new Factory(), TestHandlersObject.class);
	}
	
	public TestHandlersObject() {
		this.register(this);
	}
